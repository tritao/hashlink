...
h_bool hl_module_apply_patch( hl_module *m, hl_patch *patch, const char **error_msg ) {
	...
	for(int i=0;i<patch->function_count;i++){int slot=allocation->functions[i].findex;hl_patch_code *old=m->patch_owners[slot];hl_jit_patch_method(m->functions_ptrs[slot],m->functions_ptrs+slot);m->functions_ptrs[slot]=(unsigned char*)allocation->code+offsets[i];m->patch_owners[slot]=allocation;allocation->references++;if(old&&--old->references==0){old->next_retired=m->retired_patch_code;m->retired_patch_code=old;}}
